...
class homeControllers {
    ...
    get_products = async (req, res) => {
        try {
            const products = await productModel.find({ isActive: true }).limit(16).sort({
                createdAt: -1
            })
            const allProduct1 = await productModel.find({ isActive: true }).limit(9).sort({
                createdAt: -1
            })
            const latest_product = this.formateProduct(allProduct1);
            const allProduct2 = await productModel.find({ isActive: true }).limit(9).sort({
                rating: -1
            })
            const topRated_product = this.formateProduct(allProduct2);
            const allProduct3 = await productModel.find({ isActive: true }).limit(9).sort({
                discount: -1
            })
